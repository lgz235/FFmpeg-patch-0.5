...
sudo docker build --target build -f ${BASEDIR}/${dockerfile} --network=host \
    -t "ffmpeg-video-analytics-$build_type:v2.3" "$BASEDIR" $(env | grep -E '_(proxy|REPO|VER)=' | sed 's/^/--build-arg /')

sudo docker build -f ${BASEDIR}/${dockerfile} --network=host -t ffmpeg-video-analytics-$build_type:v2.3 \
